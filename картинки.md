...
> Підв'язані в `src/app/layout.tsx` `metadata` (`openGraph.images` + `icons`). Файли поки відсутні — `next/image` метатеги покажуть 404 поки не покладеш реальні файли. Це очікувано до часу генерації.
...
**Path:** `public/icon-512.png` (плюс `/icon-192.png` — той самий, 192×192; `/icon.svg` — векторна версія)
**Used in:** favicon set in `src/app/layout.tsx` `metadata.icons.icon`

**Prompt:**
> [STYLE ANCHOR] + Square brand mark "157" inside an octagonal ink stamp frame, deep burgundy ink, slight bleed, transparent background variant for the SVG. Square 1:1 framing.

**Negative:** photographic, gradients, glossy.

#### `brand/icon.svg`

Generate as a clean vector-friendly SVG version of the icon-512 prompt — solid path shapes, no photo style.

#### `brand/apple-touch-icon` · 180×180 · plain

**Path:** `public/apple-touch-icon.png`
**Used in:** `src/app/layout.tsx` `metadata.icons.apple`

Same as icon-512 з безпечною зоною всередині (Apple обрізає кути на iOS).
