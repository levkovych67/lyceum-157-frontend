...
> Critical UI flows (student product create/edit) — stubs. Немає FE CI, немає global security headers. API contract drift із BE.
...
- Granular categories: Essential (always on) / Functional / Analytics / Marketing.
...
- Pre-consent: блокує тільки non-essential SDK loads (analytics SDKs init delayed until consent).

**Acceptance:**
- [ ] `src/_app/providers/cookie-consent-provider.tsx` із Zustand store.
- [ ] `<CookieBanner />` у root layout.
- [ ] Analytics SDK init у `useEffect` після `analyticsConsent === true`.
...
- **P2-3** SEO structured data Product JSON-LD + sitemap.xml route + robots.txt (disallow /admin /student /parent /account).
- **P2-4** i18n: всі strings через `t()`, ніяких inline. `react/jsx-no-literals` lint.
...
| Sprint | Тижні | Зміст |
|---|---|---|
| **0** | 1 | P0-4 (CI), P0-3 (security headers), P0-API-1 (api.json sync) |
...
**Production-ready:** після Sprint 5 (всі P0 + P0+ + P1 закриті, FE CI green, a11y AA).
